...
function timeSince(timeStamp, withMins=true) {
    ...
    const formatDate = function(date, format, utc) {
        ...
        let MMMM = ["\x00", "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
        ...
        let dddd = ["\x02", "Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"];
    ...
    if(secondsPast > 172800){ // After two days
        ...
        if(secondsPast <= 604800)
            timeString = `${formatDate(timeStamp, "dddd")} ${withMins?  " at " + formatDate(timeStamp, "h:mmtt"):''}`; // with in a week
            ...
            timeString =`${formatDate(timeStamp, "MMMM d")}  ${withMins?  " at " + formatDate(timeStamp, "h:mmtt"):''}`; // with in a month
