...
storyController.updateStory = async (req, res) => {
  ...
  try {
    ...
    if (!updatedStory) { throw new Error("Maybe the story is deleted or you aren\'t the owner or Poor connection"); }
    res.status(201).json({
      success: true,
      story: updatedStory,
    });
...
storyController.deleteStory = async (req, res) => {
  ...
  try {
    ...
    if (!deletedStory) { throw new Error('You don\'t have a permission'); }
...
storyController.getStoriesByTopic = async (req, res) => {
  ...
  try {
    ...
    const stories = await db.Story.paginateRecords(
      aggregate, {
        match: params,
        project: {
          ...storyFields,
...
storyController.getPopularStories = async (req, res) => {
  ...
  const pageSize = 9;
  const aggregate = db.Story.aggregate();
  const data = await db.Story.paginateRecords(
    aggregate, {
